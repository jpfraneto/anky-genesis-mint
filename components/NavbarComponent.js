...
export default function NavbarComponent() {
  ...
  const navList = (
    <ul className="mt-2 mb-4 flex flex-col items-end gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      <Typography
        as="li"
        variant="small"
        color="blue-gray"
  ...
  return (
    <Navbar className="mx-auto text-black max-w-screen-xl px-8 py-2 lg:px-8 lg:py-4">
      <div className="container mx-auto flex items-center justify-between   text-blue-gray-900">
        <Typography
          as="a"
          href="#"
          className="mr-4 mt-1 cursor-pointer pt-1.5 text-3xl"
        >
          <Link href="/">ANKY</Link>
        </Typography>
          ...
          {authenticated ? (
            <div className="flex flex-col">
              <Button
                ...
                variant="text"
                size="xl"
                className="hidden text-2xl lg:inline-block"
              >
                <span>logout</span>
              </Button>
              <small>{thisWallet.address}</small>
            </div>
          ) : (
            <Button
              onClick={login}
        ...
        <IconButton
          variant="text"
          className="ml-auto h-6 mb-3 w-6 text-inherit hover:bg-transparent focus:bg-transparent active:bg-transparent lg:hidden"
          ripple={false}
          onClick={toggleOpen}
        >
          {open ? (
      ...
      <Collapse open={open}>
        <div className="mt-2 mb-4 container w-full ml-auto">
          {open && <>{navList}</>}
        </div>
      </Collapse>
    </Navbar>
